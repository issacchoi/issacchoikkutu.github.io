...
function query(_q){
	...
	for(i in _q) if(_q[i]||_q[i]=='') res.push(_q[i]);
...
function oQuery(_q){
	...
	for(i in _q) if(_q[i]||_q[i]=='') res.push([ i, _q[i] ]);
...
function sqlWhere(q){
	...
	function wSearch(item){
		...
		if((c = item[1]['$ltee']) !== undefined) return item[0]+"<="+c;
		if((c = item[1]['$gtee']) !== undefined) return item[0]+">="+c;

        if((c = item[1]['$raw']) !== undefined) return Escape("%K=%s", item[0], c);
		if((c = item[1]['$json']) !== undefined) return Escape("%K->>%s", item[0], c);
...
exports.Agent = function(type, origin){
	...
	this.RedisTable = function(key){
		...
		/*170415_CharX_OfflineUserRank(s)*/
		my.source = key;
		my.putData = function(id, value){
			var R = new Lizard.Tail();

			origin.hset(key, id, value, function(err, res){
				R.go(id);
			});
			return R;
		};
		my.putDataOnly = function(id, value){
			origin.hset(key, id, value, function(err, res){});
		};
		my.getData = function(arr, cb){
			origin.hmget(key, arr, cb);
		};
		/*170415_CharX_OfflineUserRank(e)*/
		...
		/*170415_CharX_RankDelta(s)*/
		my.getGlobalMulti = function(arr, cb, ret) {
			if (!arr || !arr.length) return cb([]);
			if (ret === undefined) ret = [];
			origin.zrevrank([ key, arr[ret.length] ], function(err, res){
				//console.log("Check "+ret.length+" get "+res);
				ret.push(res);
				if (ret.length+1 < arr.length) my.getGlobalMulti(arr, cb, ret);
				else cb(ret);
			});
		};
		my.snapShot = function(snapshot) {
			origin.zunionstore([ snapshot, 1, key ], function(err, res) { });
		};
		/*170415_CharX_RankDelta(e)*/
		/*170425_CharX_Competition(s)*/
		my.rem0 = function() {
			var R = new Lizard.Tail();
			origin.zremrangebyscore([ key, 0, 0 ], function(err, res) {
				R.go(null);
			});
			return R;
		};
		my.getAll = function() {
			...
			origin.zrevrange([ key, 0, -1, "WITHSCORES" ], function(err, res) {
				if (!res) { R.go([]); return; }
				var A = [];
				var i, len = res.length;
				for(i=0; i<len; i += 2)
					A.push({ id: res[i], score: res[i+1] });
				R.go(A);
			});
			return R;
		};
		my.remDataOnly = function(id) {
			origin.zrem([ key, id ]);
		};
		my.getSize = function(ret) {
			origin.zcard(key, ret);
		};
		my.increase = function(id, amount) {
			origin.zincrby([ key, amount, id ], function(err, res){});
		};
		/*170425_CharX_Competition(e)*/
	...
	this.PostgresTable = function(col){
			...
			this.on = function(f, chk, onFail){
				...
				switch(mode){
					...
					case "insert":
						...
						break;
					case "insert_":
						sql = Escape("INSERT INTO %I (%s) VALUES (%s) RETURNING id", col, sqlIK(q), sqlIV(q));
						break;
					case "update":
						if(_my.second['$inc']){
		...
		my.insert_ = function(){
			return new pointer("insert_", query(arguments));
		};
		my.update = function(){
