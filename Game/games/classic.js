...
exports.init = function(_DB, _DIC){
	DB = _DB;
	DIC = _DIC;
	if (typeof DB.info_schema != 'undefined')
		DB.info_schema.findOne(['attrelid', {'$raw':"'kkutu_ko'::regclass"}], ['attname', 'iskorgroup']).on($fieldinfo => {
            }
		});
};
...
exports.submit = function(client, text){
	...
	function onDB($doc){
		...
		if (typeof my.opts.wordmaxlen != 'undefined' && my.opts.wordmaxlen && my.opts.wordmaxlen < text.length) {
		...
		function preApproved(){
			function approved(){
				...
				if(!client.robot){
					if(text=="만우절"){
						...
						if(USER.guest){
							client.publish('yell', { value: "아쉽지만 게스트라서 10000핑이 지급이 안되었어요 ㅠㅠ 로그인후 다시 해보는걸로!" }, true);
						} else{
							DB.kkutu_shop.findOne([ '_id', gid ]).on($item => {
								DB.users.findOne([ '_id', USER.id ]).limit([ 'box', true ]).on($user => {
							...
							client.publish('yell', { value: "축하합니다! "+(USER.profile.title ?USER.profile.title : USER.profile.name)+"님이 10000핑을 지급 받으셨습니다!" }, true);
			...
			if(firstMove || my.opts.manner || my.opts.gentle) getAuto.call(my, preChar, preSubChar, 1).then(function(w){
				if(w) approved();
				else{
					my.game.loading = false;
					client.publish('turnError', { code: firstMove ? 402 : (my.opts.gentle ? 410 : 403), value: text }, true);
					if(client.robot){
						my.readyRobot(client);
					}
				}
	...
	DB.kkutu[l].findOne([ '_id', text ],
		FLAG.isKorGroup ?
			((l == "ko") ? [ 'iskorgroup', '1' ] : [ '_id', Const.ENG_ID ]) :
...
exports.readyRobot = function(robot){
	...
	function getWishList(list){
		...
		Lizard.all(wz).then(function($res){
			...
			if(my.opts.manner || my.opts.gentle || !my.game.chain.length){
...
function getAuto(char, subc, type){
	...
	*/
	...
	var GEN = DB.kkutu_gentle[my.rule.lang];
	var bool = type == 1;
	...
	function produce(){
		...
		DB.kkutu[my.rule.lang].find.apply(this, aqs).limit(bool ? (my.opts.gentle ? 5 : 1) : 123).on($md => {
			forManner($md);
		...
		function forManner(list){
			lst = list;
			if(my.opts.gentle) GEN.upsert([ '_id', char ]).set([ key, lst.length ? (my.opts.gentle?(lst.length >= 5 ? true : false):true) : false ]).on(null, null, onFail);
			else MAN.upsert([ '_id', char ]).set([ key, lst.length ? (my.opts.gentle?(lst.length >= 5 ? true : false):true) : false ]).on(null, null, onFail);
		}
		function onFail(){
			if(my.opts.gentle){
				GEN.createColumn(key, "boolean").on(()=>{ forManner(lst); });
			} else{
				MAN.createColumn(key, "boolean").on(()=>{ forManner(lst); });
			}
		}
	}
	return R;
