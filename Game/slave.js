...
Server.on('connection', function(socket){
	...
	let key = Master.validateToken(chunk[0]);
	if (!key) {
		socket.send('{"type":"error","code":999,"message":"CTRL+F5 혹은 브라우저 캐시를 삭제한 후 다시 접속해 주세요"}');
		socket.close();
		JLog.info("REFUSED BECAUSE OF FORGED TOKEN/"+ip);
		return;
	}
	...
	switch(msg.type){
			...
			if(msg.relay){
				...
				if(msg.whisper){
					...
					msg.whisper.split(',').forEach(v => {
						if(temp = DIC[DNAME[v]]){
							temp.send('chat', { from: $c.nick, profile: $c.profile, value: msg.value });
						}else{
							$c.sendError(424, v);
						}
					});
		...
		case 'setRoom':
			...
			if(stable){
				...
				if (msg.opts.onlybeginner && $c.getLevel($c.data.score) > 25) {
					msg.code = 1001;
					stable = false;
				}
		...
		case 'practice':
			if(!ROOM[$c.place]) return;
			if(ROOM[$c.place].gaming) return;
			if(ROOM[$c.place].mode==14) return;
		...
		case 'pictureQuiz':
			if(!ROOM[$c.place]) return;
			if(!ROOM[$c.place].game) return;
			if(!ROOM[$c.place].game.seq) return;
			if(ROOM[$c.place].game.seq[ROOM[$c.place].game.turn]!=$c.id) return;
			var specialColor = ["darkred","orange","aqua","purple","brightred","brightorange","indiago","slightlybrightorange","mint","brightgray","slightlybrightgray","gray","slightlydarkgray","darkgray"];
			if(msg.eventtype=="setColor")
				if(specialColor.indexOf(msg.color)!=-1)
					if(!DIC[$c.id].box.hasOwnProperty("pkg_specialpalette")) return;
			$c.publish(msg.type, msg);
			break;
		default:
			break;
	}
